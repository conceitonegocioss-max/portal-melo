...
export default function UsuariosPerfisPage() {
  ...
  const rows = useMemo(() => {
    ...
    return usuarios.filter((u) => {
      ...
      return [u.nome, u.cpf, maskCpf(u.cpf), u.empresa, u.perfil, u.perfilAuditoria, u.status].join(" ").toLowerCase().includes(query);
    });
  ...
  if (!ready) {
    return (
      <main className="section gray"><div className="container"><p>Carregando usuários…</p></div></main>
    );
  }
    ...
    <main className="section gray">
      <div className="container usuariosPage">
        <div className="topActions noPrint">
          ...
          <Link href="/colaborador/auditoria/certificacoes" className="btn btn-yellow small">Controle de Certificações</Link>
        ...
        <p className="section-text usuariosDesc">Controle administrativo visual de usuários, perfis e status de acesso. As alterações feitas abaixo são apenas para conferência na tela; o salvamento definitivo será implementado depois.</p>
      ...
      <style jsx global>{`
        ...
        .usersTable th:nth-child(1), .usersTable td:nth-child(1) { width:28%; }
        .usersTable th:nth-child(2), .usersTable td:nth-child(2) { width:14%; }
        .usersTable th:nth-child(3), .usersTable td:nth-child(3) { width:18%; }
        ...
        .usersTable th:nth-child(7), .usersTable td:nth-child(7) { width:6%; }
        ...
        @media(max-width:900px){ .summaryGrid{grid-template-columns:repeat(2,minmax(0,1fr));} .tableWrap{overflow-x:auto;} .usersTable{min-width:900px;} }
        @media print { .noPrint{display:none!important;} body,.section.gray{background:#fff!important;} .container{max-width:100%!important;} .tableWrap{box-shadow:none!important; overflow:visible!important;} .usersTable th,.usersTable td{font-size:9px!important;padding:6px!important;} .statusSelect{border:0!important;background:transparent!important;color:#000!important;appearance:none!important;} }
      `}</style>
    </main>
  );
}
